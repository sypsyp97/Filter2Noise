...
def patch_xlstm_torch_compat():
    """
    Patch torch's cpp_extension.include_paths signature for xlstm==2.0.0.

    That release calls include_paths(cuda=True), while newer torch versions use
    device_type='cuda'. We adapt the call site without touching site-packages.
    """
    include_paths = getattr(cpp_extension, "include_paths", None)
    if include_paths is None:
        return

    try:
        signature = inspect.signature(include_paths)
    ...
    def include_paths_compat(*args, cuda=None, **kwargs):
        if cuda is not None and "device_type" not in kwargs:
            kwargs["device_type"] = "cuda" if cuda else "cpu"
        return include_paths(*args, **kwargs)

    cpp_extension.include_paths = include_paths_compat


patch_xlstm_torch_compat()
...
def hilbert_rotate(side_length, x, y, rx, ry):
    """
    Rotate and flip a quadrant while converting a Hilbert index to coordinates.
    """
    if ry == 0:
        if rx == 1:
            x = side_length - 1 - x
...
def hilbert_index_to_xy(order, index):
    """
    Map a Hilbert curve index to 2D coordinates on an order x order grid.
    """
    x = 0
    y = 0
    t = index
    ...
    while side_length < order:
        rx = (t // 2) & 1
        ry = (t ^ rx) & 1
        x, y = hilbert_rotate(side_length, x, y, rx, ry)
        x += side_length * rx
        y += side_length * ry
        t //= 4
        side_length *= 2
...
def build_hilbert_order(grid_h, grid_w):
    """
    Build a Hilbert traversal for a rectangular patch grid.

    The grid is embedded into the smallest power-of-two square, traversed with a
    Hilbert curve, then cropped back to the valid rectangular region.
    """
    side = next_power_of_two(max(grid_h, grid_w))
    order = []

    for index in range(side * side):
        x, y = hilbert_index_to_xy(side, index)
        if y < grid_h and x < grid_w:
            order.append(y * grid_w + x)

    order = torch.tensor(order, dtype=torch.long)
    inverse_order = torch.empty_like(order)
    inverse_order[order] = torch.arange(order.numel(), dtype=torch.long)
...
class SigmaPredictor(nn.Module):
    ...
    using a patch-based xLSTM mechanism.

    Attributes:
        patch_size (int): The size of the non-overlapping image patches.
        hidden_dim (int): Dimension for latent representation in xLSTM.
        feature_proj (nn.Linear): Project patches to hidden dimension.
        xlstm (XLSTMStack): xLSTM stack for sequence modeling.
    ...
    def __init__(self, patch_size=8, hidden_dim=8, depth=1, num_heads=2):
        """
        Args:
            patch_size (int, optional): Patch size for non-overlapping patches. Defaults to 8.
            hidden_dim (int, optional): Hidden dimension for the xLSTM stack. Defaults to 8.
            depth (int, optional): Number of xLSTM blocks. Defaults to 1.
            num_heads (int, optional): Number of heads in the mLSTM cell. Defaults to 2.
        """
        super().__init__()
        if XLSTM_IMPORT_ERROR is not None:
            raise ImportError(
                "Official xlstm package is not available. Install it from "
                "https://github.com/NX-AI/xlstm before using filter2noise_xlstm.py"
            ) from XLSTM_IMPORT_ERROR
        ...
        self.sigma_proj = nn.Linear(self.hidden_dim, 3)
        self.activation = BoundedSoftplus(threshold=6)

    def _build_xlstm(self, context_length, device):
        mlstm_config = mLSTMLayerConfig(
            conv1d_kernel_size=4,
            qkv_proj_blocksize=max(4, self.hidden_dim),
        ...
        self.xlstm = xLSTMBlockStack(stack_config).to(device)
        self.context_length = context_length

    def _get_hilbert_order(self, grid_h, grid_w, device):
        cache_key = (grid_h, grid_w)
        if cache_key not in SigmaPredictor._hilbert_cache:
            SigmaPredictor._hilbert_cache[cache_key] = build_hilbert_order(grid_h, grid_w)

        order, inverse_order = SigmaPredictor._hilbert_cache[cache_key]
        return order.to(device), inverse_order.to(device)

    def forward(self, x):
        """
        Predict sigmas from the input image using xLSTM.
        ...
        patches = x.view(b, c, grid_h, ps, grid_w, ps)
        patches = patches.permute(0, 2, 4, 1, 3, 5).contiguous()
        patches = patches.view(b, grid_h * grid_w, -1)

        hilbert_order, inverse_order = self._get_hilbert_order(grid_h, grid_w, x.device)
        patches = patches.index_select(1, hilbert_order)

        if self.xlstm is None:
            self._build_xlstm(context_length=patches.shape[1], device=x.device)
        elif patches.shape[1] > self.context_length:
            raise ValueError(
                f"Sequence length {patches.shape[1]} exceeds initialized xLSTM context "
                f"length {self.context_length}. Warm up the model on the largest input first."
            )

        feats = self.feature_proj(patches)
        sigmas_patch = self.xlstm(feats)
        sigmas_patch = sigmas_patch.index_select(1, inverse_order)
        sigmas_patch_norm = self.norm(sigmas_patch)
        sigmas_patch_proj = self.sigma_proj(sigmas_patch_norm)
        sigmas = self.activation(sigmas_patch_proj)

        sigmas = sigmas.view(b, grid_h, grid_w, 3).permute(0, 3, 1, 2)
...
class AGBF(nn.Module):
    ...
    def __init__(self, patch_size=8, hidden_dim=8, depth=1, num_heads=2):
        ...
        """
        super().__init__()
        self.sigma_predictor = SigmaPredictor(
...
class DenoisingPipeline(nn.Module):
    ...
    def __init__(self, num_stages=2, patch_size=8, hidden_dim=8, depth=1, num_heads=2):
        """
        Args:
...
def main():
    ...
    # Warm up once on the largest input so the official xLSTM stack is built
