...
class SigmaPredictor(nn.Module):
    ...
    using a patch-based Mamba mechanism.

    Attributes:
        patch_size (int): The size of the non-overlapping image patches.
        hidden_dim (int): Dimension for latent representation in Mamba.
        feature_proj (nn.Linear): Project patches to hidden dimension.
        feature_mamba (Mamba): Mamba model for feature processing.
        sigma_mamba (Mamba): Mamba model for sigma prediction.
    ...
    def __init__(self, patch_size=8):
        ...
        # Mamba configurations and models
        feature_config = MambaConfig(d_model=self.hidden_dim, n_layers=2)
        self.feature_mamba = Mamba(feature_config)
        
        sigma_config = MambaConfig(d_model=self.hidden_dim, n_layers=2)
        self.sigma_mamba = Mamba(sigma_config)
    ...
    def forward(self, x):
        """
        Predict sigmas from the input image using Mamba.
        ...
        patches = patches.view(b, (h // ps) * (w // ps), -1)  # [B, L, patch_features]

        # Project patches to hidden dimension
        feats = self.feature_proj(patches)  # [B, L, hidden_dim]

        # Feature processing with Mamba
        feats = self.feature_mamba(feats)  # [B, L, hidden_dim]

        # Sigma prediction with Mamba
        sigmas_patch = self.sigma_mamba(feats)  # [B, L, hidden_dim]
        sigmas_patch_norm = self.norm(sigmas_patch)
        sigmas_patch_proj = self.sigma_proj(sigmas_patch_norm)
        sigmas = self.activation(sigmas_patch_proj)  # [B, L, 3]
